...
function useToast() {
  ...
  const Toast = msg ? (
    ...
    >
      <Ionicons name={type === 'success' ? 'checkmark-circle' : 'alert-circle'} size={16} color={type === 'success' ? '#34D399' : '#F87171'} />
      <Text style={{ color: type === 'success' ? '#34D399' : '#F87171', fontWeight: '600', fontSize: 14 }}>{msg}</Text>
    </Animated.View>
  ) : null;
...
function SectionCard({
  title, icon, accentColor, children, compact,
}: {
  title: string; icon: keyof typeof Ionicons.glyphMap;
  accentColor?: string; children: React.ReactNode; compact?: boolean;
}) {
  const colors    = useTheme();
  ...
  return (
    <View style={[{ backgroundColor: colors.surface, borderRadius: radii.xxl, borderWidth: 1, borderColor: colors.border, overflow: 'hidden' }, elevation]}>
      ...
      <View style={{ padding: compact ? spacing(4) : spacing(6) }}>{children}</View>
    </View>
  );
}
...
// ─────────────────────────────────────────────────────────────────────────────
function Field({
  label, icon, compact, locked, helper, containerStyle, ...inputProps
}: React.ComponentProps<typeof TextInput> & {
  label: string; icon?: keyof typeof Ionicons.glyphMap;
  compact?: boolean; locked?: boolean; helper?: string;
  ...
  return (
    <View style={[{ minWidth: 0, width: '100%' }, containerStyle]}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing(2), marginBottom: spacing(2), minHeight: 18 }}>
        <Text numberOfLines={1} style={[typography.caption, { color: colors.textMuted, letterSpacing: 0.8, fontSize: compact ? 10 : 11, fontWeight: '700', flexShrink: 1 }]}>
          {label.toUpperCase()}
        </Text>
        {locked && (
      ...
      <View style={[{ flexDirection: 'row', alignItems: 'center', minHeight: compact ? 46 : 52, borderWidth: 1, borderRadius: radii.lg, borderColor: locked ? `${colors.warning}33` : colors.border, backgroundColor: locked ? `${colors.warning}08` : colors.surfaceAlt, paddingHorizontal: spacing(compact ? 3 : 4), gap: spacing(2), overflow: 'hidden' }, elevation]}>
        {icon && <Ionicons name={icon} size={compact ? 15 : 17} color={locked ? colors.warning : colors.primary} style={{ flexShrink: 0 }} />}
        <TextInput
          {...inputProps}
          editable={!locked && inputProps.editable !== false}
          placeholderTextColor={colors.textMuted}
          numberOfLines={1}
          style={[typography.body, { flex: 1, minWidth: 0, color: locked ? colors.textMuted : colors.textPrimary, paddingVertical: spacing(compact ? 2 : 3), fontSize: compact ? 13 : 14 }]}
        />
      </View>
      {helper && <Text style={[typography.caption, { color: colors.textMuted, fontSize: compact ? 10 : 11, marginTop: spacing(1) }]}>{helper}</Text>}
    </View>
  );
}
...
// FormGrid
// ─────────────────────────────────────────────────────────────────────────────
function FormGrid({ columns, gap, children }: { columns: 1 | 2; gap: number; children: React.ReactNode }) {
  const items = React.Children.toArray(children);
  return (
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap, width: '100%' }}>
      {items.map((child, i) => (
        <View key={i} style={{ flexBasis: columns === 1 ? '100%' : '48.5%', flexGrow: columns === 1 ? 1 : 0, flexShrink: 1, minWidth: columns === 1 ? '100%' : 260, maxWidth: '100%' }}>
          {child}
        </View>
      ))}
    </View>
...
// ─────────────────────────────────────────────────────────────────────────────
// NavRow — tappable row used in both mobile quick-links and desktop sidebar
// ─────────────────────────────────────────────────────────────────────────────
function NavRow({
  icon, label, description, accent, onPress, last,
}: {
  icon:        keyof typeof Ionicons.glyphMap;
  label:       string;
  description?: string;
  ...
  return (
    ...
    >
      <View style={{ width: 40, height: 40, borderRadius: radii.md, backgroundColor: `${accent}18`, borderWidth: 1, borderColor: `${accent}33`, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        <Ionicons name={icon} size={18} color={accent} />
      </View>
      <View style={{ flex: 1, minWidth: 0 }}>
        <Text style={[typography.body, { color: colors.textPrimary, fontWeight: '600', fontSize: 14 }]}>{label}</Text>
        {description && (
          <Text style={[typography.caption, { color: colors.textMuted, marginTop: 2 }]} numberOfLines={1}>{description}</Text>
        )}
      </View>
...
// HeroStat
// ─────────────────────────────────────────────────────────────────────────────
function HeroStat({ icon, label, value, accent, compact }: { icon: keyof typeof Ionicons.glyphMap; label: string; value: string; accent?: string; compact?: boolean }) {
  const colors = useTheme();
  const c = accent ?? colors.primary;
  return (
    <View style={{ flex: 1, minWidth: compact ? 80 : 100, flexDirection: 'row', alignItems: 'center', gap: spacing(compact ? 2 : 3), paddingHorizontal: spacing(compact ? 2 : 3), paddingVertical: spacing(compact ? 2 : 3), backgroundColor: `${c}0F`, borderRadius: radii.lg, borderWidth: 1, borderColor: `${c}22` }}>
      <View style={{ width: compact ? 28 : 34, height: compact ? 28 : 34, borderRadius: radii.md, backgroundColor: `${c}22`, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        <Ionicons name={icon} size={compact ? 13 : 16} color={c} />
      </View>
      <View style={{ flex: 1, minWidth: 0 }}>
        <Text style={[typography.bodyStrong, { color: colors.textPrimary, fontSize: compact ? 12 : 14 }]} numberOfLines={1}>{value}</Text>
        <Text style={[typography.caption, { color: colors.textSecondary, marginTop: 1, fontSize: compact ? 9 : 10 }]} numberOfLines={1}>{label}</Text>
      </View>
    </View>
  );
}
...
// StatusBadge
// ─────────────────────────────────────────────────────────────────────────────
function StatusBadge({ label, icon, color, compact }: { label: string; icon: keyof typeof Ionicons.glyphMap; color?: string; compact?: boolean }) {
  const colors = useTheme();
  const c = color ?? colors.primary;
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing(1), paddingHorizontal: spacing(compact ? 2 : 3), paddingVertical: spacing(1), borderRadius: radii.pill, backgroundColor: `${c}1A`, borderWidth: 1, borderColor: `${c}44` }}>
      <Ionicons name={icon} size={compact ? 10 : 12} color={c} />
      <Text style={[typography.caption, { color: c, fontWeight: '700', letterSpacing: 0.4, fontSize: compact ? 9 : 11 }]}>{label}</Text>
    </View>
  );
}
  ...
  return (
    <View style={{ gap: spacing(2) }}>
      ...
      <View style={{ height: compact ? 5 : 7, backgroundColor: colors.border, borderRadius: radii.pill, overflow: 'hidden' }}>
        <View style={{ height: '100%', width: `${pct}%` as any, backgroundColor: color, borderRadius: radii.pill }} />
      </View>
    </View>
  );
}
...
function StudentProfileContent() {
  ...
  if (!currentUser) {
    return (
      <View style={{ flex: 1, backgroundColor: colors.background, alignItems: 'center', justifyContent: 'center', gap: spacing(5), padding: spacing(8) }}>
    ...
    <View style={[{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: padX, paddingVertical: spacing(compact ? 3 : 4), backgroundColor: colors.surface, borderBottomWidth: 1, borderBottomColor: colors.border, gap: spacing(3) }, elevMd]}>
      ...
      <Pressable
        ...
        style={({ pressed }) => ({
          ...
          paddingHorizontal: spacing(compact ? 3 : 4), paddingVertical: spacing(2),
          borderRadius: radii.lg,
          backgroundColor: !isDirty ? colors.surfaceAlt : saving ? colors.surfaceAlt : colors.primary,
          borderWidth: 1,
          borderColor: !isDirty ? colors.border : saving ? colors.border : colors.primary,
          opacity: saving || !isDirty ? 0.5 : pressed ? 0.88 : 1,
          ...Platform.select({ ios: { shadowColor: colors.primary, shadowOffset: { width: 0, height: 3 }, shadowOpacity: isDirty ? 0.3 : 0, shadowRadius: 8 }, android: { elevation: isDirty ? 3 : 0 }, web: { boxShadow: isDirty && !saving ? `0 3px 12px ${colors.primary}44` : 'none' } as any, default: {} }),
        })}
      >
  ...
  // ── Profile hero ──────────────────────────────────────────────────────────
  const ProfileHero = (
    <View style={[{ backgroundColor: colors.surface, borderRadius: radii.xxl, borderWidth: 1, borderColor: colors.border, overflow: 'hidden', marginBottom: spacing(compact ? 5 : 7), width: '100%' }, elevLg]}>
          ...
          <View style={{ flexDirection: 'row', alignItems: compact ? 'center' : 'flex-start', gap: spacing(compact ? 4 : 6) }}>
            ...
            <View style={{ flex: 1, minWidth: 0 }}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing(2), marginTop: spacing(1) }}>
                ...
                <Text style={[typography.body, { color: colors.textSecondary, fontSize: compact ? 12 : undefined }]} numberOfLines={1}>{email}</Text>
              </View>
  ...
  // ── Quick Links card (mobile + tablet — desktop uses sidebar) ─────────────
  // This is the key fix: all navigation links that were desktop-sidebar-only
  ...
  const QuickLinksCard = !isDesktop && (
    <View style={{ marginTop: spacing(compact ? 5 : 7) }}>
      <SectionCard title="Account & Navigation" icon="apps-outline" accentColor={colors.primary} compact={compact}>
        ...
        {/* Navigation links — same list as desktop sidebar */}
        <View style={{ marginHorizontal: -spacing(compact ? 4 : 6) }}>
        ...
        <View style={{ height: 1, backgroundColor: colors.divider, marginTop: spacing(2), marginBottom: spacing(4) }} />
            ...
            <View style={{ paddingHorizontal: padX, paddingTop: spacing(compact ? 5 : 7), maxWidth: 1280, alignSelf: 'center', width: '100%' }}>
              ...
              <View style={{ flexDirection: isDesktop ? 'row' : 'column', gap: compact ? spacing(5) : spacing(8), alignItems: 'flex-start' }}>
                <View style={{ flex: 1, minWidth: 0, width: '100%' }}>
                  {FormCard}
                  {/* Quick links appear here on mobile/tablet, below the form */}
                  {QuickLinksCard}
                </View>
                {Sidebar}
              </View>
