      ...
      >
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ color: '#fff', fontWeight: '700', fontSize: 14 }}>
            {saved ? t('Course saved!') : t('Course removed')}
          </Text>
          <Text style={{ color: 'rgba(255,255,255,0.8)', fontSize: 12, marginTop: 1 }}>
            {saved ? t('Find it anytime in your Saved tab') : t('Removed from your saved list')}
          </Text>
        </View>
...
function CourseCard({
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={`${t('View full course details')}: ${course.title}`}
    ...
    >
      ...
      <View style={{ padding: isMobile ? spacing(4.5) : spacing(5.5), gap: spacing(4) }}>
          ...
          <View style={{ flexDirection: 'row', gap: spacing(2) }}>
            {[
              ...
              { icon: 'desktop-outline' as const, label: course.mode },
              {
                ...
                label: `${t('Minimum')} ${course.minimumPoints || course.requiredPoints} ${t('Points')}`,
                highlight: true,
              },
        ...
        >
          ...
          >
            ...
            >
              <Text style={[typography.caption, { color: colors.primary, fontSize: 10, fontWeight: '700' }]}>
                {passedSubjects.length} {passedSubjects.length === 1 ? t('Subject') : t('SUBJECTS')} {t('matched')}
              </Text>
            </View>
          </View>
          ...
          <View style={{ padding: spacing(4), gap: spacing(3) }}>
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={[typography.caption, { color: colors.textMuted, fontSize: 11 }]}>
                        {t('Required')} {item.requiredGrade} {t('or better')} · {t('you got')} {item.studentGrade}
                      </Text>
                    </View>
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={[typography.body, { fontSize: 13, color: colors.textPrimary, fontWeight: '600' }]}>
                  {userPoints} {t('TOTAL POINTS')}
                </Text>
                <Text style={[typography.caption, { color: colors.textMuted, fontSize: 11 }]}>
                  {course.pointsMet
                    ? `${userPoints - (course.minimumPoints || course.requiredPoints)} ${t('Points')} ${t('above the minimum of')} ${course.minimumPoints || course.requiredPoints}`
                    : `${Math.abs(course.pointsDifference || 0)} ${t('Points')} ${t('below minimum of')} ${course.minimumPoints || course.requiredPoints}`}
                </Text>
              </View>
              <Ionicons
                ...
                {missingSubjects.slice(0, 3).map((item, index) => (
                  ...
                  >
                    ...
                    <Text style={[typography.caption, { color: colors.textMuted, flex: 1, lineHeight: 18, fontSize: 12 }]}>
                      {item.subject}: {t('you have')} {item.studentGrade === 'Not taken' ? t('not taken this subject') : item.studentGrade}, {t('minimum required')} {item.requiredGrade}
                    </Text>
                  </View>
                ))}
              </View>
        ...
        <View style={{ flexDirection: 'row', gap: spacing(2.5) }}>
          {[
            { icon: 'desktop-outline' as const, label: t('Mode'), value: course.mode },
            { icon: 'card-outline' as const, label: t('Tuition'), value: formatMoney(course.tuitionPerYear) },
          ].map((item, i) => (
            <View
              key={i}
              style={{
  ...
  useEffect(() => {
    ...
    const loadSavedCourses = async () => {
      ...
      try {
        ...
        console.error('Failed to load saved recommended courses:', error);
        if (active) setErrorMessage(getSavedItemsErrorMessage(error));
      }
    };
...
useEffect(() => {
  ...
  const toggleSave = async (course: Course) => {
    ...
    const wasSaved = savedCourses.has(course.id);

    setErrorMessage('');
    setSavingCourseIds((current) => new Set(current).add(course.id));
    setSavedCourses((current) => {
      const next = new Set(current);
      if (wasSaved) next.delete(course.id);
    ...
    try {
      if (wasSaved) {
        ...
        await saveItem('course', {
          id: course.id,
          title: course.title,
          institution: course.institutionName || 'Unknown Institution',
          institutionId: course.institutionId,
          institutionType: course.institutionType || 'university',
          facultyId: course.facultyId,
          facultyName: course.facultyName || '',
          duration: course.duration || 'Not specified',
          fee: formatMoney(course.tuitionPerYear),
          tuitionPerYear: course.tuitionPerYear ?? null,
          level: course.qualificationLevel || 'Programme',
          qualificationLevel: course.qualificationLevel || 'Programme',
          requiredPoints: course.minimumPoints || course.requiredPoints,
          mode: course.mode || 'Full-time',
          about: course.about || '',
          careers: course.careers || [],
          matchScore: course.matchScore || 0,
          recommendationSummary: course.recommendationSummary || '',
      ...
      console.error('Failed to update saved recommended course:', error);

      setSavedCourses((current) => {
        const next = new Set(current);
        if (wasSaved) next.add(course.id);
        else next.delete(course.id);
        return next;
      });

      setErrorMessage(getSavedItemsErrorMessage(error));
    } finally {
      setSavingCourseIds((current) => {
        const next = new Set(current);
  ...
  return (
    <View style={{ flex: 1 }}>
      <DashboardLayout
        ...
        subtitle={t('Courses matched to your academic results')}
      ...
      >
        {!!errorMessage && (
          ...
          >
            ...
            <Text style={[typography.caption, { color: colors.danger, flex: 1, lineHeight: 18 }]}> 
              {errorMessage}
            </Text>
            <Pressable
          ...
          >
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={[typography.h2, { color: '#fff', fontSize: isMobile ? 20 : 24, marginBottom: spacing(1.5) }]}>
                  {loading ? t('Finding your matches…') : `${courses.length} ${t('courses matched')}`}
                </Text>
                <Text style={{ color: '#ffffffcc', fontSize: 13, lineHeight: 20 }}>
                  {t('Each recommendation includes a full explanation of why you qualify, based on your subjects and points.')}
                </Text>
              </View>
            </View>
            ...
            {!loading && courses.length > 0 && (
              <View style={{ flexDirection: 'row', gap: spacing(2), marginTop: spacing(4) }}>
                {[
                  { icon: 'pulse-outline' as const, label: t('Avg Match'), value: `${avgScore}%` },
                  { icon: 'school-outline' as const, label: t('Universities'), value: statsUni },
                  { icon: 'library-outline' as const, label: t('Colleges'), value: statsCol },
                  { icon: 'construct-outline' as const, label: t('Brigades'), value: statsBrig },
                ].map((stat, i) => (
                  <View
                    key={i}
                    style={{
        ...
        >
          ...
          <TextInput
            ...
            placeholder={t('Search courses, institutions, faculties…')}
          ...
          />
          {searchTerm.length > 0 && (
            <Pressable onPress={() => setSearchTerm('')} accessibilityRole="button" accessibilityLabel={t('Clear Search')} hitSlop={10}>
        ...
        >
          {([
            { key: 'ALL', label: t('All Types'), icon: 'apps-outline' },
            { key: 'university', label: t('Universities'), icon: 'school-outline' },
            { key: 'college', label: t('Colleges'), icon: 'library-outline' },
            { key: 'brigade', label: t('Brigades'), icon: 'construct-outline' },
          ] as const).map((type) => {
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing(2), marginBottom: spacing(5) }}>
          ...
          {([
            { key: 'match', label: t('Best Match'), icon: 'star-outline' },
            { key: 'points', label: t('Points'), icon: 'trending-up-outline' },
            { key: 'name', label: t('A–Z'), icon: 'text-outline' },
          ] as const).map((option) => {
          ...
          >
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing(1.5) }}>
              ...
              <Text style={[typography.caption, { color: colors.textSecondary, fontSize: 12 }]}>
                {t('Showing ')}<Text style={{ fontWeight: '700', color: colors.textPrimary }}>{visibleCourses.length}</Text>{t(' of ')}<Text style={{ fontWeight: '700', color: colors.textPrimary }}>{displayedCourses.length}</Text> {t('matched courses')}
              </Text>
            </View>
        ...
        {loading ? (
          <View style={{ paddingVertical: spacing(16), alignItems: 'center', gap: spacing(4) }}>
            ...
            <View style={{ alignItems: 'center', gap: spacing(1) }}>
              <Text style={[typography.h2, { color: colors.textPrimary, fontSize: 18 }]}>
                {t('Finding your matches…')}
              </Text>
              <Text style={[typography.body, { color: colors.textSecondary, fontSize: 13 }]}>
                {t('Analysing subjects, points and requirements')}
              </Text>
            </View>
          </View>
          ...
          >
            ...
            <Text style={[typography.body, { color: colors.textMuted, textAlign: 'center', lineHeight: 21 }]}>
              {searchTerm
                ? `${t('No results for')} "${searchTerm}". ${t('Try a different search.')}`
                : t('No courses match the current filter. Try selecting a different type.')}
            </Text>
            {(searchTerm || filterType !== 'ALL') && (
              ...
              >
                ...
                <Text style={[typography.label, { color: '#fff' }]}>{t('Clear filters')}</Text>
              </Pressable>
            )}
          </View>
        ) : (
          <View style={{ gap: spacing(5) }}>
            {visibleCourses.map((course: Course) => (
              <CourseCard
                ...
                onToggleSave={() => void toggleSave(course)}
            ...
            {visibleCourses.length < displayedCourses.length && (
              ...
              >
                ...
                <Text style={[typography.label, { color: colors.primary, fontWeight: '700' }]}>
                  {t('Load')} {Math.min(6, displayedCourses.length - visibleCourses.length)} {t('more courses')}
                </Text>
              </Pressable>
            )}
            ...
            {visibleCourses.length === displayedCourses.length && displayedCourses.length > 0 && (
              <View style={{ alignItems: 'center', paddingVertical: spacing(4), gap: spacing(1.5) }}>
                ...
                <Text style={[typography.caption, { color: colors.textMuted, textAlign: 'center' }]}>
                  {t("You've seen all")} {displayedCourses.length} {t('matched courses')}
                </Text>
              </View>
            )}
          </View>
