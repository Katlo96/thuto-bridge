...
export function parseFirebaseError(error: unknown): string {
  ...
  switch (code) {
    case 'auth/email-already-in-use': return 'An account already exists for these details. Please sign in instead.';
    case 'auth/invalid-email': return 'Please enter a valid email address.';
    case 'auth/weak-password': return 'Password is too weak. Use at least 8 characters.';
    case 'auth/user-not-found':
    case 'auth/profile-not-found': return 'No account was found. Please create an account first.';
    case 'auth/wrong-password':
    case 'auth/invalid-credential':
    case 'auth/invalid-login-credentials': return 'Incorrect phone number, email address, or password.';
    case 'auth/phone-not-verified': return 'This phone number has not been verified. Complete OTP verification before signing in.';
    case 'auth/user-disabled': return 'This account has been disabled. Please contact support.';
    case 'auth/too-many-requests': return 'Too many verification attempts were made. Please wait before requesting another OTP.';
    case 'auth/network-request-failed': return 'Network error. Please check your connection and try again.';
    case 'auth/invalid-verification-code': return 'Invalid OTP code. Please check the code and try again.';
    case 'auth/invalid-verification-id':
    case 'auth/session-expired': return 'The verification session expired. Please request a new OTP.';
    case 'auth/code-expired': return 'The OTP code expired. Please request a new one.';
    case 'auth/invalid-phone-number': return 'Enter a valid Botswana phone number, for example 71 234 567 or +267 71 234 567.';
    case 'auth/missing-phone-number': return 'Please enter a phone number.';
    case 'auth/quota-exceeded': return 'The SMS limit has been reached. Please try again later.';
    ...
    case 'auth/not-phone-account': return 'This phone number is not linked to a password-based account. Please sign in with your original method.';
    default: return value?.message || 'Something went wrong. Please try again.';
  }
}
...
export function normalisePhone(raw: string): string {
  ...
  if (!phone.startsWith('+267')) throw authError('auth/invalid-phone-number', 'Thuto Bridge currently accepts Botswana phone numbers only.');
...
export async function signUpWithPhonePassword(phoneInput: string, password: string, fullName: string): Promise<{ user: User; phone: string }> {
  ...
  let user: User;
  try {
    user = (await createUserWithEmailAndPassword(auth, alias, password)).user;
  } catch (error: any) {
    if (error?.code !== 'auth/email-already-in-use') throw error;
    user = (await signInWithEmailAndPassword(auth, alias, password)).user;
    await reload(user);
    if (user.phoneNumber) {
      await signOut(auth).catch(() => undefined);
  ...
  pendingPhone = phone;
  storedVerificationId = await new PhoneAuthProvider(auth).verifyPhoneNumber(phone, getRecaptchaVerifier());
  return { user, phone };
}

export async function resendPhoneSignupOTP(phoneInput: string): Promise<void> {
  const user = auth.currentUser;
  if (!user || !isPhonePasswordEmail(user.email)) throw authError('auth/session-expired');
  const phone = normalisePhone(phoneInput);
  pendingPhone = phone;
  storedVerificationId = await new PhoneAuthProvider(auth).verifyPhoneNumber(phone, getRecaptchaVerifier());
}

export function getStoredVerificationId(): string | null { return storedVerificationId; }
export function clearStoredPhoneVerification(): void { storedVerificationId = null; pendingPhone = null; }
...
export async function completePhonePasswordSignup(otpCode: string, fullName?: string): Promise<{ user: User }> {
  ...
  const credential = PhoneAuthProvider.credential(storedVerificationId, otpCode);
  ...
  await setDoc(doc(db, 'students', linked.user.uid), {
    phone: pendingPhone,
    phoneVerified: true,
...
export async function forgotPassword(email: string): Promise<void> { await sendPasswordResetEmail(auth, email.trim().toLowerCase()); }
...
export async function sendPasswordResetOTP(phoneInput: string): Promise<{ phone: string }> {
  ...
  resetVerificationId = await new PhoneAuthProvider(auth).verifyPhoneNumber(phone, getRecaptchaVerifier());
  return { phone };
}
...
export async function verifyPasswordResetOTP(otpCode: string): Promise<void> {
  ...
  const credential = PhoneAuthProvider.credential(resetVerificationId, otpCode);
...
export function subscribeToAuthState(callback: (user: User | null) => void): AuthUnsubscribe { return onAuthStateChanged(auth, callback); }
...
export async function waitForAuthenticatedUser(timeoutMs = 5000): Promise<User | null> {
  ...
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => { if (!finished) { finished = true; unsubscribe(); resolve(auth.currentUser); } }, timeoutMs);
    unsubscribe = onAuthStateChanged(auth, (user) => { if (!finished) { finished = true; clearTimeout(timeout); unsubscribe(); resolve(user); } });
  });
}
export async function logOut(): Promise<void> { clearStoredPhoneVerification(); clearStoredPasswordResetVerification(); await signOut(auth); }